...
│   ├── kanidm/         # Identity provider (admin passwords, OIDC secrets)
...
Service secrets are stored in `secrets/services/<service-name>/` and are manually created using `ragenix`. Unlike machine secrets, these are typically one-time setup.

### Kanidm (Identity Provider)

Kanidm requires admin passwords and OIDC client secrets for each integrated application.

**Required secrets:**

| Secret | Purpose |
|--------|---------|
| `admin-password.age` | Root admin account password |
| `idm-admin-password.age` | IDM admin account password (user management) |
| `oidc-<app>.age` | OAuth2/OIDC client secret for each application |
...
# Generate admin passwords (strong random passwords)
openssl rand -base64 32 | ragenix --editor - -e secrets/services/kanidm/admin-password.age
openssl rand -base64 32 | ragenix --editor - -e secrets/services/kanidm/idm-admin-password.age

# Generate OIDC client secrets for each application
openssl rand -base64 32 | ragenix --editor - -e secrets/services/kanidm/oidc-nextcloud.age
openssl rand -base64 32 | ragenix --editor - -e secrets/services/kanidm/oidc-jellyfin.age
openssl rand -base64 32 | ragenix --editor - -e secrets/services/kanidm/oidc-immich.age
openssl rand -base64 32 | ragenix --editor - -e secrets/services/kanidm/oidc-paperless.age
```
...
1. Generate the secret:
   ```bash
   openssl rand -base64 32 | ragenix --editor - -e secrets/services/kanidm/oidc-<newapp>.age
   ```

2. Declare in `nixos-secrets.nix`:
   ```nix
   kanidm-oidc-<newapp> = mkIf (elem hostname servers) {
     file = ./secrets/services/kanidm/oidc-<newapp>.age;
     owner = "kanidm";
     group = "kanidm";
     mode = "0400";
   };
...
3. Add to access control in `secrets.nix`:
   ```nix
   "secrets/services/kanidm/oidc-<newapp>.age".publicKeys = servers;
   ```

4. Configure in `modules/nixos/gumdrop/kanidm.nix`:
   ```nix
   systems.oauth2.<newapp> = {
     displayName = "New App";
     originUrl = "https://newapp.example.com";
     originLanding = "https://newapp.example.com";
     basicSecretFile = config.age.secrets.kanidm-oidc-<newapp>.path;
     # ... other settings
   };
   ```

5. Re-key secrets: `ragenix --rekey`

**Configuring applications:**

After deployment, configure each application's OIDC settings:

| Setting | Value |
|---------|-------|
| Issuer/Authority | `https://auth.lucasr.com/oauth2/openid/<client-name>` |
| Client ID | `<client-name>` (e.g., `nextcloud`, `jellyfin`) |
| Client Secret | Contents of the corresponding `oidc-<app>.age` file |

To retrieve a client secret for application configuration:
```bash
age -d -i ~/.ssh/id_ed25519 secrets/services/kanidm/oidc-<app>.age
```

### Vaultwarden

Password manager secrets:
- `env.age` - Environment variables including admin token

```bash
# Generate admin token (Argon2 hashed)
ADMIN_TOKEN=$(openssl rand -base64 32)
echo "ADMIN_TOKEN=$(echo -n "$ADMIN_TOKEN" | argon2 $(openssl rand -base64 32) -id -e)" | \
  ragenix --editor - -e secrets/services/vaultwarden/env.age
```

### Nextcloud

- `db-pass.age` - PostgreSQL database password

```bash
openssl rand -base64 32 | ragenix --editor - -e secrets/services/nextcloud/db-pass.age
```

### Immich

- `api-key.age` - API key for external integrations
- `secretsFile` - Additional secrets (DB password, etc.)

```bash
openssl rand -base64 32 | ragenix --editor - -e secrets/services/immich/api-key.age
```
